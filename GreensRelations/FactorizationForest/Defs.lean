...
lemma buildXSeq_ge (a : S) {α : Type*} [LinearOrder α] [Fintype α]
    (σ : MultiplicativeLabeling S α) (w : α) (y : α) (hy : y ∈ buildXSeq a σ w) :
    w ≤ y := by
  classical
  rw [buildXSeq] at hy
  ...
  · rcases List.mem_cons.mp hy with rfl | hy_tail
    · exact le_rfl
    · have h_y_next : Finset.min' _ h ≤ y := buildXSeq_ge a σ (Finset.min' _ h) y hy_tail
      have h_w_lt : w < Finset.min' _ h := (Finset.mem_filter.mp (Finset.min'_mem _ h)).2.1
      exact le_trans (le_of_lt h_w_lt) h_y_next
  ...
  decreasing_by
    ...
    have h_mem := Finset.min'_mem _ h
    obtain ⟨_, h_x_lt_y, _⟩ := Finset.mem_filter.mp h_mem
    have h_le : Finset.univ.filter
      (fun z => (Finset.min' _ h) < z) ⊆ Finset.univ.filter (fun z => w < z) :=
      fun _ hz => Finset.mem_filter.mpr ⟨(Finset.mem_filter.mp hz).1, lt_trans h_x_lt_y
        (Finset.mem_filter.mp hz).2⟩
    have h_ne : Finset.univ.filter
      (fun z => (Finset.min' _ h) < z) ≠ Finset.univ.filter (fun z => w < z) :=
      fun heq => lt_irrefl _ (Finset.mem_filter.mp (heq.symm ▸ Finset.mem_filter.mpr
        ⟨Finset.mem_univ _, h_x_lt_y⟩ : (Finset.min' _ h) ∈ _)).2
    exact Finset.card_lt_card (lt_of_le_of_ne h_le h_ne)
...
lemma buildXSeq_range_w (a : S) {α : Type*} [LinearOrder α] [Fintype α]
    ...
    IsGreenD (σ.σ x y) a := by
  classical
  rw [buildXSeq] at hx hy
  ...
  · let w' := Finset.min' _ h
    ...
    | Or.inl hx_eq, Or.inr hy_tail => hx_eq ▸ isGreenD_of_prefix a σ h_img w w' y hw'.1
      (buildXSeq_ge a σ w' y hy_tail) hw'.2
    | Or.inr hx_tail, Or.inl hy_eq => by
      rw [hy_eq] at hlt
      nomatch (lt_irrefl w (lt_trans
      (lt_of_lt_of_le hw'.1 (buildXSeq_ge a σ w' x hx_tail)) hlt))
    | Or.inr hx_tail, Or.inr hy_tail => buildXSeq_range_w a σ h_img w' x hx_tail y hy_tail hlt
...
lemma buildXSeq_gap_not_D (a : S) {α : Type*} [LinearOrder α] [Fintype α]
    ...
    ¬ IsGreenD (σ.σ ((buildXSeq a σ w).get ⟨i, hi_lt⟩) y) a := by
  classical
  intro h_D
  generalize h_xs : buildXSeq a σ w = xs at hi_lt h_lt h_gt h_D ⊢
  have h_eq : buildXSeq a σ w =
    if h_cond : (Finset.univ.filter (fun y => w < y ∧ IsGreenD (σ.σ w y) a)).Nonempty then
      w :: buildXSeq a σ (Finset.min' _ h_cond)
    else [w] := by rw [buildXSeq]
  ...
  · let w' := Finset.min' _ h
    ...
    | succ i' =>
      ...
      exact buildXSeq_gap_not_D a σ h_img w' i' hi_tail y h_lt
        (fun hi_succ_lt => h_gt (Nat.succ_lt_succ hi_succ_lt)) h_D
  · simp only [dif_neg h] at h_xs
    subst h_xs
    cases i with
    | zero =>
...
lemma buildXSeq_strict_mono (a : S) {α : Type*} [LinearOrder α] [Fintype α]
    (σ : MultiplicativeLabeling S α) (w : α) (i j : ℕ)
    (hi_lt : i < (buildXSeq a σ w).length) (hj_lt : j < (buildXSeq a σ w).length) (hij : i < j) :
    (buildXSeq a σ w).get ⟨i, hi_lt⟩ < (buildXSeq a σ w).get ⟨j, hj_lt⟩ := by
  classical
  generalize h_xs : buildXSeq a σ w = xs at hi_lt hj_lt ⊢
  have h_eq : buildXSeq a σ w =
    if h_cond : (Finset.univ.filter (fun y => w < y ∧ IsGreenD (σ.σ w y) a)).Nonempty then
      w :: buildXSeq a σ (Finset.min' _ h_cond)
    else [w] := by rw [buildXSeq]
  ...
  · let w' := Finset.min' _ h
    ...
    | zero =>
      ...
      | succ j' =>
        ...
        exact lt_of_lt_of_le (Finset.mem_filter.mp (Finset.min'_mem _ h)).2.1
          (buildXSeq_ge a σ w' _ (List.mem_iff_get.mpr ⟨⟨j', hj_tail⟩, rfl⟩))
    | succ i' =>
      cases j with
      ...
      | succ j' =>
        have hi_tail : i' < (buildXSeq a σ w').length := Nat.succ_lt_succ_iff.mp hi_lt
        have hj_tail : j' < (buildXSeq a σ w').length := Nat.succ_lt_succ_iff.mp hj_lt
        exact buildXSeq_strict_mono a σ w' i' j' hi_tail hj_tail (Nat.succ_lt_succ_iff.mp hij)
  · simp only [dif_neg h] at h_xs
    subst h_xs
    cases i with
    | zero =>
